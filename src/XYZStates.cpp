...
void XYZStates::add_to_rmf(ParticleIndex, RMF::NodeHandle parent) const {
  RMF::decorator::ParticleFactory f(parent.get_file());
  for (unsigned int i = 0; i < states_.size(); ++i) {
    std::ostringstream oss;
    oss << i;
    RMF::NodeHandle n = parent.add_child(oss.str(), RMF::REPRESENTATION);
    RMF::decorator::Particle d = f.get(n);
    d.set_mass(1.0);
    d.set_coordinates(RMF::Vector3(states_[i]));
  }
}
void XYZStates::update_rmf(ParticleIndex, RMF::NodeHandle parent,
  ...
  for (unsigned int i = 0; i < states_.size(); ++i) {
    ...
    RMF::decorator::Particle d = f.get(children[i]);
    d.set_radius(r);
  }
}
